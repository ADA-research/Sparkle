...
def test_irace_configure(mock_add_to_queue: Mock) -> None:
    ...
    runs = irace_conf.configure(
        conf_scenario, data_target=data_target, validate_after=False, base_dir=base_dir
    )
...
def test_irace_organise_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    source_path = Path(
        "tests/test_files/Configuration/test_output_irace.Rdata"
    ).absolute()
    ...
    assert IRACE.organise_output(source_path, None, None, 1) == {
        ...
        "sel_clause_div": "2",
        "sel_var_div": "6",
        "prob_first_div": "0.9918",
        "gamma_hscore2": "495644",
        "prob_novelty": "0.4843",
    }


def test_irace_scenario_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_irace_scenario_from_file() -> None:
    ...
    scenario_file = Path(
        "tests/test_files/Configuration/PbO-CCSAT-Generic_PTN_scenario_irace.txt"
    )
