...
def test_smac3_scenario_to_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_smac3_scenario_from_file() -> None:
    ...
    assert scenario.instance_set.directory == Path(
...
def test_smac3_configure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Test SMAC3 scenario configuration."""
    solver = Solver(Path("tests/test_files/Solvers/Test-Solver").absolute())
    solver_relative_path = Solver(Path("tests/test_files/Solvers/Test-Solver"))
    instance_set = Instance_Set(Path("tests/test_files/Instances/Train-Instance-Set"))
    objectives = [resolve_objective("PAR10"), resolve_objective("accuray:min")]
...
def test_organise_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Test SMAC3 configuration output organisation."""
    file = Path(
        "tests/test_files/Configuration/results/"
        "runhistory_PbO-CCSAT-Generic_PTN_SMAC3.json"
    ).absolute()
    solver = Solver(Path("tests/test_files/Solvers/Test-Solver").absolute())
    instance_set = Instance_Set(
        Path("tests/test_files/Instances/Train-Instance-Set").absolute()
