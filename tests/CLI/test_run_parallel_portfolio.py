...
portfolio_path = Path(
    "tests/test_files/Output/Parallel_Portfolio/Raw_Data/runtime_experiment/"
)
stdout = (
...
def test_run_parallel_portfolio(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    [r_default_objective_values, r_cpu_time, r_status, r_wall_time] = (
        rpp.init_default_objectives()
    )
    ...
    with (
        patch("sparkle.CLI.run_parallel_portfolio.time.sleep", return_value=None),
        patch("sparkle.CLI.run_parallel_portfolio.tqdm") as mock_tqdm,
        patch(
            "sparkle.CLI.run_parallel_portfolio.rrr.add_to_queue"
        ...
        returned_run = rpp.submit_jobs(
            returned_cmd, solvers, instance_file, Runner.SLURM
        )
        assert type(returned_run) is SimpleNamespace

        job_output_dict = rpp.monitor_jobs(
            returned_run, instance_file, solvers, r_default_objective_values
        )
        assert type(job_output_dict) is dict

        assert rpp.wait_for_logs(returned_cmd) is None

        job_output_dict = rpp.update_results_from_logs(
            returned_cmd, returned_run, solvers, job_output_dict, r_cpu_time
        )
        assert type(job_output_dict) is dict

        job_output_dict = rpp.fix_missing_times(
            job_output_dict, r_status, r_cpu_time, r_wall_time
        )
        assert type(job_output_dict) is dict
        ...
        for instance_pair in pdf.instance_pairs:
            ...
            assert instance_name in single_instances_str, (
...
def test_parser_function() -> None:
    ...
    expected_description = "Run a portfolio of solvers on an instance set in parallel."
