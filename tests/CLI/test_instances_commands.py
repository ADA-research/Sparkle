...
def test_add_remove_instances_iterable_file_instance_set(
    ...
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        add_instances.main([str(instances_path.absolute()), "--no-copy"])
    assert pytest_wrapped_e.type is SystemExit
    assert pytest_wrapped_e.value.code == 0
...
def test_add_remove_instances_multi_file_instance_set(
    ...
    instances_path = (
        Path("Examples") / "Resources" / "CCAG" / "Instances" / "CCAG"
    ).absolute()
    monkeypatch.chdir(tmp_path)  # Execute in PyTest tmp dir
    # Smoke test
    with pytest.raises(SystemExit) as pytest_wrapped_e:
...
@pytest.mark.integration
def test_add_remove_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
