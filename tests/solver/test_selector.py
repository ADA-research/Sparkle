...
def test_construct(mock_add_queue: Mock) -> None:
    ...
    performance_data_path = Path("tests/test_files/Output/Performance_Data/performance_data.csv")
    ...
    selector.construct(target_file, performance_data, feature_data, objective, solver_cutoff, run_on, sbatch_options, base_dir)
    ...
    selector_performance.shape[1] == performance_data.num_solvers + 1  # +1 for the instance column
...
def test_run(instance: str) -> None:
    ...
    feature_data = FeatureDataFrame(Path("tests/test_files/Output/Feature_Data/feature_data.csv"))
