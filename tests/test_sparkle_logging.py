...
import Commands.sparkle_help.sparkle_logging as sl
import Commands.sparkle_help.sparkle_global_help as sgh
...
def test_add_output() -> None:
    ...
    with Path(sl.caller_log_path).open("r") as output_file:
...
def test_log_command() -> None:
    ...
    argv = ["test.py"]
    sl.log_command(argv)
    with Path(sgh.sparkle_global_log_path).open("r") as log_file:
