...
class RandomForest:
    ...
    def configspace(self: RandomForest) -> ConfigurationSpace:
        ...
        use_max_depth = EqualsCondition(
            child=max_depth, parent=max_depth_type, value="int"
        )
        ...
        max_features_type = Categorical(
            "max_features_type", ["special", "float"], default="special"
        )
        max_features_special = Categorical(
            "max_features_special", ["sqrt", "log2", "None"], default="sqrt"
        )
        # TODO max val to max features in dataset
        max_features_float = Float("max_features_float", (0.0, 1.0), default=0.5)

        use_max_features_special = EqualsCondition(
            child=max_features_special, parent=max_features_type, value="special"
        )
        use_max_features_float = EqualsCondition(
            child=max_features_float, parent=max_features_type, value="float"
        )

        max_leaf_nodes_type = Categorical(
            "max_leaf_nodes_type", ["None", "int"], default="None"
        )
        max_leaf_nodes = Integer("max_leaf_nodes", (1, 1000), default=100)
        use_max_leaf_nodes = EqualsCondition(
            child=max_leaf_nodes, parent=max_leaf_nodes_type, value="int"
        )
        ...
        max_samples_type = Categorical(
            "max_samples_type", ["None", "float"], default="None"
        )
        max_samples = Float("max_samples", (0.05, 0.95), default=0.8)

        use_max_samples = EqualsCondition(
            child=max_samples, parent=max_samples_type, value="float"
        )
        use_max_samples2 = EqualsCondition(
            child=max_samples_type, parent=bootstrap, value=True
        )
        ...
        cs.add(
            [
                use_max_depth,
                use_max_features_float,
                use_max_features_special,
                use_max_leaf_nodes,
                use_oob_score,
                use_max_samples,
                use_max_samples2,
