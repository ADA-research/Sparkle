...
class CCAGInstanceFeature:
    feature_names = ["num_way", "num_option", "average_option_value",
                     "num_constraint", "average_constraint_length"]
    ...
    def get_ccag_features(self):
        feature_values = [self.num_way, self.num_option, self.average_option_value,
                          self.num_constraint, self.average_constraint_length]
...
parser.add_argument("-instance_file", nargs="+", type=str,
                    help="Paths to the instance files, grouped in order as [.model, .constraints]")
parser.add_argument("-output_file", type=str, help="Path to the output file")
args = parser.parse_args()

if args.features:
    # Return a list of feature names and their feature groups as [(feature_group, feature_name), ...]
    print([(FeatureGroup.BASE.value, name)
          for name in CCAGInstanceFeature.feature_names])
    sys.exit()

ccag_model_file_path = Path(
    next((inst for inst in args.instance_file if ".model" in inst), None))
ccag_constraint_file_path = Path(
    next((inst for inst in args.instance_file if ".constraints" in inst), None))
ccag = CCAGInstanceFeature(Path(args.extractor_dir),
                           ccag_model_file_path, ccag_constraint_file_path)
