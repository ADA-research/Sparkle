...
def print_command(instance_file, seed_str: str, cutoff_time_str: str):
	...
	# NOTE: This algorithm requires the cutoff time, but to ensure consistent runtime measurements by Sparkle we multiply it by 10 to make sure the algorithm does not stop early, but is stopped by Sparkle (runsolver) consistenty with other solvers
