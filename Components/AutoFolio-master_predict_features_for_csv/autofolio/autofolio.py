...
# added by luo
from .sparkle_csv.sparkle_feature_data_csv_help import Sparkle_Feature_Data_CSV # added by luo
...
class AutoFolio(object):
    ...
    def run_cli(self):
        ...
        if args_.load:
            ...
            # added by luo
            if args_.feature_csv:
                feature_csv_path = args_.feature_csv
                feature_csv = Sparkle_Feature_Data_CSV(feature_csv_path)
                for instance in feature_csv.list_rows():
                    feature_vector_list = feature_csv.get_feature_vector_list(instance)
                    print(instance)
                    self.read_model_and_predict(model_fn=args_.load, feature_vec=feature_vector_list)
            # end
    ...
    def read_model_and_predict(self, model_fn: str, feature_vec: list):
        ...
        print(pred["pseudo_instance"]) # added by luo
