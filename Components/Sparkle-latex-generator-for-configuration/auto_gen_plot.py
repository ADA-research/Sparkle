...
if __name__ == "__main__":
    if len(sys.argv) != 7 and len(sys.argv) != 9:
        print(f"Usage: {sys.argv[0]} <data_solver_configured_vs_default_filename> <penalty_time> "
    ...
    fout.write(f"set arrow from {lower_bound},{lower_bound} to {upper_bound},{upper_bound} nohead lc rgb 'black'"
               + "\n")
    # Only plot cutoff boundaries for PAR10, they are not meaningful for QUALITY
    # performance
    if performance_measure == "PAR10":
        # Cutoff time x axis
        fout.write(f"set arrow from {penalty_time},{lower_bound} to {penalty_time},{upper_bound} nohead lc rgb 'black' lt 2"
                   + "\n")
        # Cutoff time y axis
        fout.write(f"set arrow from {lower_bound},{penalty_time} to {upper_bound},{penalty_time} nohead lc rgb 'black' lt 2"
                   + "\n")
    fout.write('set terminal postscript eps color dashed linewidth "Helvetica" 20\n')
    fout.write(f"set output '{output_eps_file}'" + "\n")
    fout.write(f"plot '{data_solver_configured_vs_default_filename}' with points pt 2 ps 2"
               + "\n")
    fout.close()
