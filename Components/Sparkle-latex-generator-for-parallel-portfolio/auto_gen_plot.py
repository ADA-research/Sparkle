...
if __name__ == r'__main__':
	...
	if len(sys.argv)!=6:
		print('Usage: %s <data_parallel_portfolio_sparkle_vs_vbs_filename> <penalty_time> <vbs_name> <parallel_portfolio_sparkle_name> <figure_parallel_portfolio_sparkle_vs_vbs_filename>' % (sys.argv[0]))
		exit(-1)
	
	data_parallel_portfolio_sparkle_vs_vbs_filename = sys.argv[1]
	...
	figure_parallel_portfolio_sparkle_vs_vbs_filename = sys.argv[5]
	
	output_eps_file = figure_parallel_portfolio_sparkle_vs_vbs_filename + r'.eps'
	output_pdf_file = figure_parallel_portfolio_sparkle_vs_vbs_filename + r'.pdf'
	
	vbs_name = vbs_name.replace(r'/', r'_')
	output_gnuplot_script = parallel_portfolio_sparkle_name + r'_vs_' + vbs_name + r'.plt'
	print('AUTOGEN DEBUG ' + penalty_time)
	...
	fout.write('set title \'%s vs %s\'' % (parallel_portfolio_sparkle_name, vbs_name)  + '\n')
	...
	fout.write('plot \'%s\' with points pt 2 ps 2' % (data_parallel_portfolio_sparkle_vs_vbs_filename)  + '\n')
	fout.close()
