...
### Checks
1. Freshly install the conda environment. Remove the old one with `deactivate` followed by deleting the directory and create it again with `env` followed by `pip install -e .`. Run `pip install dev-requirements.txt` for dev tools such as ruff and pytest.
