...
def run_parallel_portfolio(instances: list[Path],
    ...
    while not all(instances_done):
        ...
        # Get the job statusses
        for j in run.jobs:
            print(f"[{j.slurm_job_id}] {j.status}")
        job_status_list = [r.status == Status.COMPLETED for r in run.jobs]
        # The jobs are sorted by instance
        for i, instance in enumerate(instances):
            if instances_done[i]:
                continue
            if any(job_status_list[i*num_solvers:(i+1)*num_solvers]):
    ...
    #if perf_m == PerformanceMeasure.RUNTIME:
    #handle_waiting_and_removal_process(instances, str(parallel_portfolio_log), run.run_id,
    #                                    solver_instance_list, run.script_filepath,
    #                                    num_jobs / len(instances))

    #current_time = datetime.datetime.now().strftime("%H:%M:%S")
    #with parallel_portfolio_log.open("a+") as outfile:
    #    fcntl.flock(outfile.fileno(), fcntl.LOCK_EX)
    ...
    tmp_res_files = glob.glob(f"{gv.pap_performance_data_tmp_path}/*.result")
