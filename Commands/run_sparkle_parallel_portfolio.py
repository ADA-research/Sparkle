...
if __name__ == r'__main__':
    ...
    parser.add_argument("--instance-paths", default=sgh.instance_list, metavar='N', nargs="+", type=str, action=ac.SetByUser, help='Specify the instance_path(s) on which the portfolio will run. This can be a space-separated list of instances contain instance sets and/or singular instances. For example --instance-paths Instances/PTN/Ptn-7824-b01.cnf Instances/PTN2/')
    parser.add_argument("--portfolio-name", default=sgh.latest_scenario.get_parallel_portfolio_path(),type=str, action=ac.SetByUser, help='Specify the name of the portfolio. If the portfolio is not in the standard folder, use its full path, the default location is Sparkle_Parallel_Portfolio/. If the option is not specified, the latest constructed portfolio will be selected.')
    parser.add_argument("--process-monitoring", choices=processMonitoring.__members__, default=sgh.settings.get_parallel_portfolio_process_monitoring(), action=ac.SetByUser, help='Specify whether the monitoring of the portfolio should cancel all solvers within a portfolio once a solver finishes(realistic). Or allow all solvers within a portfolio to get an equal chance to have the shortest running time on an instance(extended). The default option is realistic.')
