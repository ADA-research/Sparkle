...
if __name__ == r'__main__':
    ...
    parser.add_argument("--instance-paths", default=sgh.instance_list, metavar='N', nargs="+", type=str, action=ac.SetByUser, help='Specify the instance_path(s) on which the portfolio will run. This can be a space seperated list of instances contain instance sets and/or singular instances. For example --instance-paths Instances/PTN/Ptn-7824-b01.cnf Instances/PTN2/')
    parser.add_argument("--portfolio-name", default=sgh.latest_scenario.get_parallel_portfolio_path(),type=str, action=ac.SetByUser, help='Specify the name of the portfolio, if the portfolio is not in the standard location use its full path, the standard location is Sparkle_Parallel_Portfolio/. If the option is not used the latest constructed portfolio will be used.')
    parser.add_argument("--process-monitoring", choices=processMonitoring.__members__, default=sgh.settings.get_parallel_portfolio_process_monitoring(), action=ac.SetByUser, help='Specify wheneter the monitoring of the portfolio should cancel all solvers within a portfolio once a solver finishes(realistic), or allow all solvers within a portfolio to get an equal chance to have the lowest amount of running time on an instance(extended). The default option is realistic.')
    parser.add_argument('--performance-measure', choices=PerformanceMeasure.__members__, default=sgh.settings.get_general_performance_measure(), action=ac.SetByUser, help='the performance measure, e.g. runtime(for decision algorithms) or qualty_absolute(for optimisation algorithms)')
    parser.add_argument("--cutoff-time", default=sgh.settings.get_general_target_cutoff_time(), type=int, action=ac.SetByUser, help='The duration the portfolio will run before the solvers within the portfolio will be stopped')
