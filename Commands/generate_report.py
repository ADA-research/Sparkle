...
if __name__ == r'__main__':
	...
	## Selection arguments
	parser.add_argument('--selection', action='store_true', help='set to generate a normal selection report')
	...
	if not selection and test_case_directory == None and solver == None:
		scenario = sgh.latest_scenario.get_latest_scenario()
		if scenario == Scenario.SELECTION:
			selection = True
			test_case_directory = sgh.latest_scenario.get_selection_test_case_directory()
	...
	if selection or test_case_directory != None:
		...
		if (flag_instance_set_train and flag_instance_set_test):
			instance_set_train_name = sfh.get_last_level_directory_name(str(instance_set_train))
			instance_set_test_name = sfh.get_last_level_directory_name(str(instance_set_test))
