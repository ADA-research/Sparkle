...
if __name__ == r'__main__':
	...
	raw_result_path = test_case_dir + r'TMP/' + key_str + r'.rawres'
	processed_result_path = test_case_dir + r'Performance_Data/TMP/' + key_str + r'.result'
	
	task_run_status_path = test_case_dir + r'TMP/SBATCH_Solver_Jobs/' + key_str + r'.statusinfo'
	...
	while True:
		try:
			fout = open(performance_data_csv_path, 'r+')
			fcntl.flock(fout.fileno(), fcntl.LOCK_EX)
			performance_data_csv.dataframe = pd.read_csv(performance_data_csv_path, index_col=0)
			if verify_string == r'SAT':
				performance_data_csv.set_value(instance_path, solver_path, runtime)
			elif verify_string == r'UNSAT':
				performance_data_csv.set_value(instance_path, solver_path, runtime)
			elif verify_string == r'UNKNOWN':
				performance_data_csv.set_value(instance_path, solver_path, penalty_time)
			else:
				performance_data_csv.set_value(instance_path, solver_path, penalty_time)
