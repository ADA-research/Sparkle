...
def get_dict_sparkle_portfolio_selector_penalty_time_on_each_instance(portfolio_selector_path, performance_data_csv_path, feature_data_csv_path, cutoff_time_each_run, par_num, test_case_dir):
	mydict = {}
	...
	file_predict_schedule_path = test_case_dir + sfh.get_last_level_directory_name(portfolio_selector_path) + '_predict_schedule_for_each_instance.txt'
...
def analyse_sparkle_portfolio_selector_with_all_solvers(portfolio_selector_path, performance_data_csv_path, feature_data_csv_path, cutoff_time_each_run, par_num, test_case_dir):
	tmp_portfolio_selector_path = portfolio_selector_path
	tmp_portfolio_selector_penalty_time_on_each_instance_path = test_case_dir + sfh.get_last_level_directory_name(tmp_portfolio_selector_path) + '_penalty_time_on_each_instance.txt'
	
	if os.path.exists(tmp_portfolio_selector_path) and os.path.isfile(tmp_portfolio_selector_path):
		while True:
			try:
				tmp_dict = get_dict_sparkle_portfolio_selector_penalty_time_on_each_instance(tmp_portfolio_selector_path, performance_data_csv_path, feature_data_csv_path, cutoff_time_each_run, par_num, test_case_dir)
...
def analyse_sparkle_portfolio_selector_excluding_one_solver(portfolio_selector_path, performance_data_csv_path, feature_data_csv_path, cutoff_time_each_run, par_num, excluded_solver, test_case_dir):
	...
	tmp_performance_data_csv_path = test_case_dir + r'TMP/' + r'tmp_performance_data_csv_' + sparkle_basic_help.get_time_pid_random_string() + r'.csv'
	tmp_performance_data_csv.save_csv(tmp_performance_data_csv_path)
	tmp_portfolio_selector_path = portfolio_selector_path + '_excluding_' + sfh.get_last_level_directory_name(excluded_solver)
	tmp_portfolio_selector_penalty_time_on_each_instance_path = test_case_dir + sfh.get_last_level_directory_name(portfolio_selector_path) + '_excluding_' + sfh.get_last_level_directory_name(excluded_solver) + '_penalty_time_on_each_instance.txt'
	...
	if len(tmp_performance_data_csv.list_columns()) >= 1:
		...
		if os.path.exists(tmp_portfolio_selector_path) and os.path.isfile(tmp_portfolio_selector_path):
			while True:
				try:
					tmp_dict = get_dict_sparkle_portfolio_selector_penalty_time_on_each_instance(tmp_portfolio_selector_path, tmp_performance_data_csv_path, feature_data_csv_path, cutoff_time_each_run, par_num, test_case_dir)
					tmp_par2_value = calc_par2_value_for_dict(tmp_dict, cutoff_time_each_run, par_num)
...
if __name__ == '__main__':
	...
	if excluded_solver == '':
		analyse_sparkle_portfolio_selector_with_all_solvers(portfolio_selector_path, performance_data_csv_path, feature_data_csv_path, cutoff_time_each_run, par_num, test_case_dir)
	else:
		analyse_sparkle_portfolio_selector_excluding_one_solver(portfolio_selector_path, performance_data_csv_path, feature_data_csv_path, cutoff_time_each_run, par_num, excluded_solver, test_case_dir)
