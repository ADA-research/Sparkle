...
def add_log_statement_to_file(logging_file: str, line: str, jobtime: str):
    ...
    convert_to_seconds = sum(int(x) * 60 ** i for i, x in enumerate(
    ...
    fo.write(f'starting time: {start_time_formatted} end time: {current_time} job '
             f'number: {line}\n')
    fo.close()
...
def log_computation_time(logging_file: str, job_nr: str, job_duration: str):
    ...
    if ':' in str(job_duration):
        convert_to_seconds = sum(int(x) * 60 ** i for i, x in enumerate(
            reversed(str(job_duration).split(':'))))
...
def remove_temp_files_unfinished_solvers(solver_array_list: list,
                                         sbatch_script_name: str, temp_solvers: list):
    ...
    for jobs in result.stdout.strip().split('\n'):
        ...
        if (sgh.settings.get_paraport_process_monitoring()
            ...
            if jobid in pending_job_with_new_cutoff and jobstatus == 'R':
                current_seconds = sum(int(x) * 60 ** i for i, x in enumerate(
                    reversed(jobtime.split(':'))))
        ...
        for finished_job in finished_job_array:
            ...
            if (int(int(job[job.find('_')+1:])/int(portfolio_size))
                ...
                if (sgh.settings.get_paraport_process_monitoring()
                    ...
                    current_seconds = sum(int(x) * 60 ** i for i, x in enumerate(
                        reversed(remaining_jobs[job][0].split(':'))))
                    cutoff_seconds = sum(int(x) * 60 ** i for i, x in enumerate(
                        reversed(newCutoffTime.split(':'))))
    ...
    for instance_path in instance_path_list:
        for solver in solver_list:
            if " " in solver:
                ...
                tmp_solver_instances.append(
                    f'{str(sfh.get_last_level_directory_name(str(solver_path)))}_seed_')
                ...
                for instance in range(1, int(seed)+1):
                    commandline = (f' --instance {str(instance_path)} --solver '
                                   ...
                                   f'{str(performance)} --seed {str(instance)}')
                    parameters.append(str(commandline))
                    solver_array_list.append(
                        f'{str(sfh.get_last_level_directory_name(str(solver_path)))}'
                        f'_seed_{str(instance)}_'
                        f'{str(sfh.get_last_level_directory_name(str(instance_path)))}')
            else:
                solver_path = Path(solver)
                solver_array_list.append(
                    f'{sfh.get_last_level_directory_name(str(solver_path))}_'
                    f'{sfh.get_last_level_directory_name(str(instance_path))}')
                commandline = (f' --instance {str(instance_path)} --solver '
                               f'{str(solver_path)} --performance-measure '
                               f'{str(performance)}')
                parameters.append(str(commandline))
...
def handle_waiting_and_removal_process(instances: list, logging_file: str,
                                       job_number: str, solver_array_list: list,
                                       sbatch_script_name: str, portfolio_size: int,
                                       remaining_job_list: list,
    ...
    finished_jobs, pending_job_with_new_cutoff, started = wait_for_finished_solver(
        logging_file, job_number, solver_array_list, remaining_job_list,
    ...
    remaining_job_list, pending_job_with_new_cutoff = cancel_remaining_jobs(
        logging_file, job_number, finished_jobs, portfolio_size, solver_array_list,
        pending_job_with_new_cutoff)

    # If there are still unfinished jobs recursively handle the remaining jobs.
    if len(remaining_job_list):
        handle_waiting_and_removal_process(instances, logging_file, job_number,
                                           solver_array_list, sbatch_script_name,
...
def run_parallel_portfolio(instances: list, portfolio_path: Path) -> bool:
    ...
    parameters, num_jobs, solver_array_list, temp_solvers = generate_SBATCH_job_list(
        solver_list, instances, performance, num_jobs)

    # Generates a SBATCH script which uses the created parameters
    sbatch_script_name, sbatch_script_path = generate_parallel_portfolio_sbatch_script(
        parameters, num_jobs)
    ...
    try:
        ...
        if(performance == 'RUNTIME'):
            handle_waiting_and_removal_process(instances, file_path_output1, job_number,
                                               solver_array_list, sbatch_script_name,
            ...
            # remove_temp_files_unfinished_solvers(solver_array_list, sbatch_script_name,
            #                                      temp_solvers)
        else:
            done = False
            ...
            while not done:
                # Ask the cluster for a list of all jobs which are currently running
                result = subprocess.run(['squeue', '--array', '--jobs', job_number],
                                        capture_output=True, text=True)
        ...
        print('c an error occurred when running the parallel algorithm portfolio please '
              'check your input or \n ')
