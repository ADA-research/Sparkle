...
def run_solvers_parallel(flag_recompute, flag_also_construct_selector_and_report=False):
	num_job_in_parallel = serh.num_job_in_parallel

	if flag_recompute:
		performance_data_csv = spdcsv.Sparkle_Performance_Data_CSV(sgh.performance_data_csv_path)
		performance_data_csv.clean_csv()
		run_solvers_parallel_jobid = srsp.running_solvers_parallel(sgh.performance_data_csv_path, num_job_in_parallel, 2)
	else:
