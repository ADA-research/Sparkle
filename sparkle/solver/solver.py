...
class Solver(SparkleCallable):
    ...
    def port_pcs(self: Solver, port_type: str) -> bool:
        ...
        if port_type == "IRACE":
            if target_pcs_file.exists():  # Already exists, possibly user defined
                return True
            # TODO: Not sure if IRACE accepts .pcs extension, examples have .txt
            # Create pcs table
            header = ["# name", "switch", "type", "values",
                      "[conditions (using R syntax)]"]
            rows = []
            constraints = [c for c in parser.pcs.params if c["type"] == "constraint"]
            for param in [p for p in parser.pcs.params if p["type"] == "parameter"]:
                ...
                if condition_str == "|":
                    condition_str = ""
                rows.append([param["name"], "--" + param["name"], param["structure"][0],
                             f"({','.join(param['domain'])})", condition_str])
            table = tabulate.tabulate(rows, headers=header, tablefmt="plain",
                                      numalign="left")
            with target_pcs_file.open("w") as f:
                f.write(f"### Parameter file for {self.name}, generated by Sparkle\n",
                        table,
                        "\n")
            return True
        return False
